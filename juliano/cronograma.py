...
reportes_pasantias = [
    ...
    # ── SEMANA 5 ────────────────────────────────────────────────
    {
        ...
        "objetivo": "Completar los módulos visuales de consulta e historial de movimientos del sistema, e integrar la lógica de visualización dinámica de expedientes por proceso administrativo, asistiendo en paralelo en las labores de revisión y canalización de correspondencia presidencial.",
        ...
        "actividades": [
            ...
            ("2", "Revisión y canalización de correspondencia presidencial", "Apoyo administrativo en la revisión ortográfica y de formato de oficios recibidos, y canalización hacia el Presidente para su firma y posterior despacho al departamento destinatario.", "Expedientes físicos, hojas de control de la oficina"),
            ("3", "Integración del historial de movimientos", "Implementación del panel de historial que muestra la línea de tiempo completa de cada expediente: fecha de ingreso, revisión, firma y egreso, con indicación del responsable de cada etapa.", "Entorno de desarrollo, base de datos SQLite con datos reales"),
            ("4", "Pruebas de usabilidad del módulo de consulta", "Ejecución de casos de prueba sobre el módulo de búsqueda utilizando datos reales de expedientes registrados en semanas anteriores, verificando la exactitud y rapidez de los resultados.", "Equipo informático, datos históricos de la base de datos"),
    ...
    # ── SEMANA 6 ────────────────────────────────────────────────
    {
        ...
        "objetivo": "Empaquetar la aplicación de escritorio mediante el framework Wails con integración de WebView2 portable, logrando un ejecutable autónomo sin dependencias de instalación, y colaborar en las labores regulares de despacho de correspondencia presidencial.",
        "entregables": "Ejecutable autónomo del sistema (.exe) empaquetado con Wails y WebView2 portable, funcional en el equipo corporativo sin requerir instalación de dependencias externas.",
        "actividades": [
            ("1", "Configuración del entorno de empaquetado con Wails", "Instalación y configuración del framework Wails en el entorno de desarrollo, revisión de compatibilidad con el sistema operativo del equipo corporativo y definición de los parámetros de compilación.", "Framework Wails, Go runtime, editor de código fuente"),
            ("2", "Apoyo en el despacho de correspondencia presidencial", "Colaboración en el registro de salida de expedientes firmados durante el período y apoyo en el envío físico hacia los departamentos de destino, con actualización del control en la hoja de cálculo.", "Expedientes firmados, hoja de cálculo institucional"),
            ("3", "Integración de WebView2 portable en el ejecutable", "Incorporación del componente WebView2 portable al proceso de compilación de Wails para garantizar la renderización de la interfaz HTML sin requerir la instalación previa de WebView2 en el equipo del usuario.", "Framework Wails, WebView2 Evergreen Standalone"),
            ("4", "Compilación y prueba del ejecutable autónomo", "Generación del archivo ejecutable final, prueba de su funcionamiento sobre el equipo corporativo de la oficina y verificación de que todos los módulos responden correctamente sin instalaciones adicionales.", "Equipo corporativo de la oficina, ejecutable compilado"),
            ("5", "Corrección de errores post-empaquetado", "Identificación y corrección de los errores detectados durante las pruebas del ejecutable: rutas relativas del archivo SQLite, resolución de recursos estáticos y ajustes de permisos de lectura y escritura.", "Editor de código fuente, repositorio GitHub")
        ]
    },
    # ── SEMANA 7 ────────────────────────────────────────────────
    {
        ...
        "actividades": [
            ("1", "Diseño del plan de pruebas funcionales", "Elaboración de la matriz de casos de prueba que cubre los flujos críticos del sistema: registro de ingreso, actualización de estatus, consulta por filtros, generación de reporte y cierre de expediente.", "Documento de requerimientos, plantilla de casos de prueba"),
            ("2", "Apoyo en elaboración de resúmenes de estatus", "Asistencia al personal del departamento en la elaboración manual de los resúmenes de expedientes solicitados por la alta dirección, aplicando los registros de la hoja de cálculo institucional.", "Hoja de cálculo preexistente, expedientes del período"),
            ("3", "Ejecución de pruebas sobre datos reales", "Corrida sistemática de los casos de prueba definidos utilizando los expedientes registrados del Departamento de Presidencia, documentando el resultado de cada caso y clasificando los errores encontrados.", "Sistema ejecutable, base de datos con datos reales"),
            ("4", "Depuración y corrección de errores detectados", "Análisis de los errores registrados durante las pruebas, corrección en el código fuente y ejecución de pruebas de regresión para confirmar que las correcciones no introducen nuevas fallas.", "Editor de código fuente, repositorio GitHub"),
    ...
    # ── SEMANA 8 ────────────────────────────────────────────────
    {
        ...
        "actividades": [
            ...
            ("2", "Participación en el cierre administrativo del período", "Apoyo en la organización final de expedientes del ciclo, verificación del registro de todos los movimientos en la hoja de cálculo y actividades regulares de soporte al departamento.", "Expedientes físicos, hoja de cálculo institucional"),
    ...
    # ── SEMANA 9 (FINAL) ────────────────────────────────────────
    {
        ...
        "actividades": [
            ...
            ("3", "Preparación de los anexos definitivos", "Organización y presentación formal de los anexos del informe: diagrama Entidad-Relación, capturas de la interfaz del sistema y memoria fotográfica del área de Presidencia.", "Impresora, archivos de imagen, editor de texto"),
